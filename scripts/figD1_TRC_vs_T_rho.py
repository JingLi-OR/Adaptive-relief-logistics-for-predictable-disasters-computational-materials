# -*- coding: utf-8 -*-
"""
Figure D.1: TRC versus |T| under different rho values
...
df = df[["rho", "T", "total_cost", "TRC", "is_lowest_TRC"]].dropna(
    subset=["rho", "T", "TRC"]
)

# Convert data types
df["rho"] = pd.to_numeric(df["rho"]).astype(int)
df["T"] = pd.to_numeric(df["T"]).astype(int)
df["total_cost"] = pd.to_numeric(df["total_cost"])
df["TRC"] = pd.to_numeric(df["TRC"])
...
# ===== 7. Save figure =====
fig.tight_layout()
fig.savefig("Figure_D1_TRC_vs_T_rho.pdf", dpi=300, bbox_inches="tight")
plt.show()